...
const CardEvent = ({ path, events, title, sortText, color, high, highBody }) => {
    ...
    const renderedItem = (pathname) => {
        switch(`${pathname}`){
            case '/':
                return (<CardItem events={events} sortText={sortText} titleSize={'text-3xl'} textSize={'text-2xl'} />)
            default:
    ...
    const renderedCard = () => {
        return (
            <div className={`max-w-full overflow-hidden bg-${color}-200 rounded-xl shadow-lg ${high}`}>
