...
const EventItem = ({ events, updateStatus, deleteEvent }) => {
    ...
    const filterButton = filterText => {
        setText(filterText)
        console.log(text)
    }

    const checkFilter = (event, text) => {
        if(text === 'all') {
            console.log(text)
    ...
    const renderedList = events
        .sort((a, b) => a.start.localeCompare(b.start))
            .filter(event => checkFilter(event, text)).map(event => {
                return (
                    <tr key={event._id}>
            ...
            <div className="flex flex-col">
                <div className="-my-2 px-8 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        ...
                        <Alert />
                            <div className="grid grid-cols-4 gap-4 justify-items-center border mb-2">
                                <div className="border">
                                    <button onClick={() => filterButton('')} type="button">new</button>
                                </div>
                                <div className="border">
                                    <button onClick={() => filterButton('upcoming')} type="button">upcoming</button>
                                </div>
                                <div className="border">
                                    <button onClick={() => filterButton('ongoing')} type="button">ongoing</button>
                                </div>
                                <div className="border">
                                    <button onClick={() => filterButton('done')} type="button">done</button>
                                </div>
                            </div>
